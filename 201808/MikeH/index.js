...
const ACTIONS = [
	{ key: 'taking-1',       income: 1, cost: 0, priority: 6, requiredCard: false        , blockableBy: []                        },
	{ key: 'foreign-aid',    income: 2, cost: 0, priority: 5, requiredCard: false        , blockableBy: ['duke']                  },
	{ key: 'couping',        income: 0, cost: 7, priority: 2, requiredCard: false        , blockableBy: []                        },
	{ key: 'taking-3',       income: 3, cost: 0, priority: 3, requiredCard: 'duke'       , blockableBy: []                        },
	{ key: 'assassination',  income: 0, cost: 3, priority: 1, requiredCard: 'assassin'   , blockableBy: ['contessa']              },
	{ key: 'swapping',       income: 0, cost: 0, priority: 7, requiredCard: 'ambassador' , blockableBy: []                        },
	{ key: 'stealing',       income: 0, cost: 2, priority: 4, requiredCard: 'captain'    , blockableBy: ['captain', 'ambassador'] },
];
...
class BOT {
	...
	processEvent (event) {
		...
		if (event.type === 'challenge-round') { // Challange
			...
			player.claimsToHave = player.claimsToHave.filter(c => c !== cardToRemove);
		}

		if (event.type === 'counter-round') { // Counter Challange
			const player = this.gameState.players[event.challengee];
			if (!player) return;
			if (event.lying) player.hasLied = true;
			// remove the required card from player's claimsToHave
	...
	chanceOfCard(targetCard, playerNames, { discardedCards, myCards, otherPlayers }) {
		...
		const claimingTC = otherPlayers.filter(p =>
	...
	OnTurn({ history, myCards, myCoins, otherPlayers, discardedCards }) {
		this.updateState({ history, myCards, myCoins, otherPlayers, discardedCards });
		...
		const priorityActions = possibleActions.sort((a, b) => a.priority - b.priority);
		...
		if( myCoins > 6 ) {
			...
			const actionObj = priorityActions.find((a) => {
				...
				case 'stealing':
					...
					targetPlayer = worthyTargets.reduce(playerWithMostCardsAndCoins, null);
				...
				case 'foreign-aid':
					...
					const chanceOfDuke = this.chanceOfCard('duke', otherPlayers.map(p => p.name),{ discardedCards, myCards, otherPlayers });
				...
				default:
					...
					targetPlayer = otherPlayers.reduce(playerWithMostCardsAndCoins, null);
					against = targetPlayer.name;
	...
	OnChallengeActionRound({ history, myCards, myCoins, otherPlayers, discardedCards, action, byWhom, toWhom }) {
		...
		if (action === 'stealing' && toWhom === MY_NAME) {
			...
			if (myCards.includes('ambassador') || myCards.includes('captain')) return false;
			// otherwise ... toss a coin.
			const chanceOfCap = this.chanceOfCard('captain', [byWhom], { myCards, otherPlayers, discardedCards });
			if (chanceOfCap < 0.2) return true;
		}

		if (action === 'taking-3') {
			const chanceOfDuke = this.chanceOfCard('duke', [byWhom], { myCards, otherPlayers, discardedCards });
			if (chanceOfDuke === 0) return true;
		}

		if (action === 'swapping') {
			const chanceOfAmbassador = this.chanceOfCard('ambassador', [byWhom], { myCards, otherPlayers, discardedCards });
	...
	OnCounterActionRound({ history, myCards, myCoins, otherPlayers, discardedCards, action, byWhom, toWhom, card }) {
		...
		const chanceOfCard = this.chanceOfCard(card, [byWhom], { discardedCards, myCards, otherPlayers });
