...
class BOT {
	...
	OnTurn({ history, myCards, myCoins, otherPlayers, discardedCards }) {
		...
		if (
			this.count('captain', discardedCards) >= 2 &&
		...
		if (
			!letsSteal ||
			(myCards.length == 1 && !myCards.includes('captain')) ||
			(myCoins >= 3 && !myCards.includes('captain'))
		) {
			this.removeAction('stealing', actionsAvailable);
		}
		...
		if (
			(myCards.includes('ambassador') || myCards[0] == myCards[1]) &&
			actionsAvailable.includes('swapping')
		) {
		...
		if (action == 'stealing') {
			otherPlayers.some((player) => {
				if (
					(player.coins >= 2) &
					!this.ifCounteredPreviously(history, player, 'stealing')
				) {
					against = player.name;
					return true;
				}
	...
	OnCounterAction({
		...
		if (action === 'assassination') {
			if (
				...
				(myCards.length === 1 && this.count('contessa', discardedCards) < 1)
			) {
				return 'contessa';
			}
			return false;
